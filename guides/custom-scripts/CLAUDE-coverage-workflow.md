...
`scripts/claude-scripts/progress.json` — per-coverpoint status (`completed`, `in_progress`, `blocked`, `not_started`).
`scripts/claude-scripts/coverage-status.md` — flat summary for quick review.

## If stuck

After 2 failed attempts, write a note to `scripts/claude-scripts/coverage_issues/<coverpoint_name>.md` and stop.
