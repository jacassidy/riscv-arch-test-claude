...
Use `pre_test_scratch_regs=N` and `{s0}`/`{s1}`/… placeholders for any scalar scratch register needed in `pre_test_lines` or `pre_instruction_lines`. `writeTest` allocates them via the central `scalar_registers_used` tracker, after `handleSignaturePointerConflict`. **Never write `x{N}` literals or hand-pick a "safe" register** — see `GUIDE.md` § Pre-test assembly for why (it can collide with a switched sigReg and cause a hang).
