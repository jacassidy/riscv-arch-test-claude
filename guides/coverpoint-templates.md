# Coverpoint Template Reference

Coverpoint templates live in the main repo at `generators/coverage/src/covergroupgen/templates/` (scalar) and `…/templates/vector/`. This guide covers file naming, format rules, copy-paste patterns, and the `ins` object API. This is the single source of truth — do not duplicate this content elsewhere.
...
If your template requires a specific LMUL (e.g. cross bins use `vtype_lmul_2`),
do NOT add a Python-side filter in `generate.py` that drops the coverpoint
when `NF × EMUL > 8`. Instead, in the CSV, mark the cell as `sew_ge{N}`
(smallest SEW where the instruction is legal at the template's required
LMUL). See `guides/csv-editing.md` → "Cell Value Semantics".

Reserved-encoding instructions (e.g. `vlseg8e64.v` at SEW=8) should have
their `EFFEW{N}` cell blanked in the CSV — the instruction is filtered out
by the existing EFFEW mechanism, no extra code needed.
